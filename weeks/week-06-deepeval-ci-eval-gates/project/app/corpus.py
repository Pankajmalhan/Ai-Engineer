...
the same fixture Week 5 used, kept here as its own copy since each week's project is a
standalone deliverable (own pyproject.toml/venv, no cross-week imports). Deliberately
small and hand-writable so every fact used by app/dataset.py's goldens can be traced
back to exactly one passage.
"""
